...
var SectionAndNoteListRenderer = ListRenderer.extend({
    ...
    _renderBodyCell: function (record, node, index, options) {
        ...
        if (isSection && record.model != 'sale.order.line' || isNote) {
        ...
        if ((isSection && record.model == 'sale.order.line')) {
            if (node.attrs.widget === "handle") {
                    return $cell;
                } else if (node.attrs.name === "aa_group") {
                    var nbrColumns = 1;
                    if (this.handleField) {
                        nbrColumns--;
                    }
                    ...
                    $cell.attr('colspan', nbrColumns);
                } else if (node.attrs.name === "aa_section_quantity") {
                    var nbrColumns = 4;
                    if (this.handleField) {
                        nbrColumns--;
                    }
                    ...
                    var nbrColumns = this._getNumberOfCols() - 2;
                    if (this.handleField) {
                        nbrColumns--;
                    }
                    if (this.addTrashIcon) {
    ...
    _onAddRecord: function (ev) {
        ...
        this.unselectRow().then(function () {
            ...
            var activeId = self._getActiveId();
            var activeModel = self._getActiveModel();
            var params = self._getPram();
            ...
            // else if (context && pyUtils.py_eval(context).open_product_variants){
            //     $('.o_add_varient').trigger('click');
            //     self._rpc({
            //         model: 'ir.model.data',
            //         method: 'xmlid_to_res_id',
            //         kwargs: {xmlid: 'task_advance.aa_sale_manage_variant_form'},
            //     }).then(function (res_id) {
            //         self.do_action({
            //             name: _t('Add Or Modify Variants'),
            //             type: 'ir.actions.act_window',
            //             res_model: 'aa.sale.manage.variant',
            //             views: [[res_id, 'form']],
            //             target: 'new',
            //             context: {
    ...
    _getActiveModel: function () {
        var saleOrderForm = this.getParent() && this.getParent().getParent();
        var stateData = saleOrderForm && saleOrderForm.state && saleOrderForm.state.model;
        var active_model = stateData;
        return active_model;
    },
    _getActiveId: function () {
        var saleOrderForm = this.getParent() && this.getParent().getParent();
        var stateData = saleOrderForm && saleOrderForm.state && saleOrderForm.state.data;
        var active_id = stateData.id;
        return active_id;
    },
    _getPram: function () {
        var saleOrderForm = this.getParent() && this.getParent().getParent();
        var stateData = saleOrderForm && saleOrderForm.state && saleOrderForm.state.context;
        var params = stateData.params;
        return params;
    },
